CREATE FUNCTION StructuredError.ErrorLookup(@Proc sql_variant, @ErrorName VARCHAR(128), @xmlArgs XML = NULL)
RETURNS VARCHAR(max)
AS
/**********************************************************************************************************************************
    DESCRIPTON:
    ...
    PARAMETERS:
        @Proc                       Procedure id from @@PROCID or name of the procedure that owns the error. 
                                    Name of the current procedure can be obtained from OBJECT_NAME(@@PROCID).
...
BEGIN
    ...
    ;
    IF CONVERT(varchar(32), SQL_VARIANT_PROPERTY(@Proc, 'BaseType')) IN ('int','bigint')
    BEGIN
        -- @Proc contains procedure identifier.
        SET @ProcID = CONVERT(int, @Proc);
        SET @ProcedureName = OBJECT_NAME(@ProcID);
    END
    ELSE
    BEGIN
        -- @Proc contains procedure name.
        SET @ProcedureName = CONVERT(varchar(255), @Proc);
    END
